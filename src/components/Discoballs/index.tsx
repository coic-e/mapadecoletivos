...
interface IDiscoballProps {
  index: number;
  z: number;
  speed: number;
}

interface IDiscoballsProps {
  speed: number;
  count: number;
  depth: number;
...
function Discoball({ index, z, speed }: IDiscoballProps) {
  const ref = useRef() as React.RefObject<THREE.LOD>;
...
export default function Discoballs({
  ...
  return (
    ...
    >
      ...
      {Array.from(
        { length: count },
        (_, i) => <Discoball key={i} index={i} z={Math.round(easing(i / count) * depth)} speed={speed} /> /* prettier-ignore */
      )}
